...
#     2) Dos máquinas virtuales Windows Server 2022 con IIS:
...
#   - Instala IIS con páginas personalizadas en cada VM
#
# PREREQUISITOS:
...
LOG_FILE="$HOME/deploy-app-gateway.log"
...
WINDOWS_IMAGE="Win2022Datacenter"          # Imagen de Windows Server 2022
...
echo "🚀 Iniciando despliegue de Application Gateway..."
...
# ----- Reglas del NSG -----
# Permiten tráfico HTTP (80) y RDP (3389) entrante.
...
echo "  ✅ Reglas NSG configuradas (HTTP:80, RDP:3389, AppGw Health:65200-65535)."

# =====================================================================
# FASE 2: MÁQUINAS VIRTUALES
# =====================================================================
# Crea dos VMs Windows Server 2022 en la subred SubNetBackendPool:
...
echo "💻 Creando/verificando VM Imagenes (Windows Server 2022)..."
...
echo "💻 Creando/verificando VM Video (Windows Server 2022)..."
...
# =====================================================================
# FASE 3: INSTALACIÓN DE IIS
# =====================================================================
# Instala IIS (Internet Information Services) en ambas VMs Windows
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "  FASE 3: INSTALACIÓN DE IIS"
echo "═══════════════════════════════════════════════════════════════"

echo "🌐 Instalando IIS en VmImagenes..."
...
<div class="container">
  ...
  <p class="server">VmImagenes - ImagesPool (Windows + IIS)</p>
...
"@
    Set-Content -Path "C:\inetpub\wwwroot\index.html" -Value $indexHtml -Encoding UTF8

    # Crear directorio /imagenes para el path routing
    New-Item -Path "C:\inetpub\wwwroot\imagenes" -ItemType Directory -Force
    $imagenesHtml = @"
<!DOCTYPE html>
<html>
...
"@
    Set-Content -Path "C:\inetpub\wwwroot\imagenes\index.html" -Value $imagenesHtml -Encoding UTF8

    # Eliminar la página por defecto de IIS
    Remove-Item -Path "C:\inetpub\wwwroot\iisstart.htm" -Force -ErrorAction SilentlyContinue
  ' \
  --output none
echo "  ✅ IIS instalado en VmImagenes."

echo "🌐 Instalando IIS en VmVideo..."
az vm run-command invoke \
  --resource-group "$RESOURCE_GROUP" \
  --name "$VM_VIDEO_NAME" \
  --command-id RunPowerShellScript \
...
<div class="container">
  ...
  <p class="server">VmVideo - VideosPool (Windows + IIS)</p>
  <p>Estud-IA - Laboratorio Application Gateway</p>
  <div class="gallery">
...
"@
    Set-Content -Path "C:\inetpub\wwwroot\videos\index.html" -Value $videosHtml -Encoding UTF8

    # Eliminar la página por defecto de IIS
    Remove-Item -Path "C:\inetpub\wwwroot\iisstart.htm" -Force -ErrorAction SilentlyContinue
  ' \
  --output none
echo "  ✅ IIS instalado en VmVideo."
...
echo "   ├── VM Imágenes:           $VM_IMAGENES_NAME"
echo "   └── VM Video:              $VM_VIDEO_NAME"
echo ""
echo "🌐 Acceso:"
